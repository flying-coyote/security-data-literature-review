...
**Which schema-on-read rate this row uses**: the schema-on-read bands above are the discounted full-stack rate from the Step 2 anchor note, $620-870/GB/day/year for platform plus Enterprise Security, while Steps 3, 4 and 6 compare MOAR against the platform-rate floor of $684K/year at 2 TB/day (the $600K of discounted-platform licensing from Step 2 plus the $84K S3 archive that sits on top of it), so the $219K annual savings, the 32% figure, the roughly 10% hybrid saving in Step 4, and the 2.6-year payback are all the conservative case, and running that same $465K MOAR total against this row's $1.24M-$1.74M band puts the modeled saving nearer 62-73% instead.
