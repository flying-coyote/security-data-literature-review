...
There's a decade of missed adoption worth naming, because it shapes how this collaboration has to work. While enterprise data-engineering teams spent 2015 to 2025 mastering modular open architectures (Iceberg, dbt, Airflow), security teams watched from behind a compliance wall built out of real regulation: SOX Section 404's separation of duties keeps security audit logs apart from business analytics data, PCI DSS v4.0 requirement 10.3.1 limits read access to audit-log files to those with a job-related need, HIPAA §164.312(b) requires audit controls, and GDPR Article 32 keeps logging separated from operational data. So security teams couldn't collaborate with data engineering, missed the Spark/Airflow/dbt wave as it happened, and are now adopting modern stacks at what I'd put at five to seven years behind their enterprise counterparts, a rough read from the deployments I've seen rather than a measured figure.
...
The incremental-modernization material (Chapter 7 of the handbook) closes on the federated summary, a staggered rollout running one and a third to two times the integrated timeline, and this section walks that rollout BU by BU. It uses the same composite that the modularity chapter sets up, a multi-BU conglomerate drawn from patterns across federated migrations rather than a single named engagement, so the timelines, uptime figures, and dollar amounts here are illustrative of how a federated rollout behaves rather than the audited results of one program; the named BUs (BU-A, BU-K, and the rest) are stand-ins for the kinds of business unit that show up in these rollouts. The binding constraint is that you can't force a BU's cutover date, so BU-A, confident after 9 months of parallel running, cuts over at month 10, while BU-K demands 5 months plus stress testing and cuts over at month 12, both valid for the risk each carries.

Four factors decide whether a federated rollout holds together. The first is that each BU controls its own timeline. Corporate wanted all 7 BUs cut over by month 6, the BUs insisted on cutting over when ready, and the workable compromise is a staggered timeline from month 10 to month 12, with BU-A in pharma at month 10, BU-C in consumer health needing 6 months of parallel and cutting over at month 10, and BU-K in vaccines demanding 5 months plus two weeks of stress testing and cutting over at month 12. BU risk tolerance differs, and forcing BU-K onto the month-6 date doesn't accelerate it, it makes BU-K exit and the coalition collapse behind it.
