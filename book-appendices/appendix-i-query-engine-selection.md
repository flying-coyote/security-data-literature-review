...
Pushing the same four engines over a 100,000,000-row slice of the same table (2026-06-07, same single host, `./moar bench 100000000`, again the median of four trials in milliseconds with the coefficient of variation in parentheses) reveals the crossover the smaller snapshot could only point at:
...
Everything else in this appendix sits downstream of a streaming layer the architecture has so far taken as given: Kafka in front, a materialization step in the middle, the engines reading Iceberg underneath. That default is under real pressure in 2026 from engines that promise to collapse the three-system pipeline into one, and a security architect gets asked about the candidates by name, so this section assesses the ones that keep coming up. **None of them has been measured in the SDW lab.** The lab's benchmarks (the flagship rerun, workload interference, cost-to-serve retention, the read-scan arms) all start after events have landed, so every claim below is a criteria-based assessment from documented public sources, cited and tier-labeled, and the numbers are the sources' own rather than mine. Use it to decide what deserves a pilot, not as a substitute for measuring your own workload.

The candidates are best read not as competing feature lists but as different bets on how state, storage, and compute should be arranged, following Anton Borisov's September 2025 framing (data architect at Fresha; Tier B), which distinguishes separation of concerns, hybrid tiered storage, and stateless compute over shared storage, extended here with batch-streaming convergence and the minimal-footprint edge log as a fifth shape.
...
The public record's biggest gap, and the reason this section carries no performance table: no methodology-disclosed benchmark exists comparing Flink, RisingWave, and Spark Real-Time Mode against OCSF-shaped security event data at SIEM scale. The closest public artifact is the Atome fraud-detection case, which is directionally relevant and not a substitute. Until that benchmark exists (it would be a new instrument class for this lab, since every current bench sits downstream of the streaming layer), the verdicts above rest on architecture analysis and adoption evidence, which is exactly why they are phrased as pilot filters rather than rankings.

The long-form assessments behind this section are on the site: "Streaming Decision: RisingWave, Fluss, and the Alternatives" (the four-philosophy comparison, the modeled cost scenario, and the migration-risk analysis), "Kafka to Iceberg: Hidden Integration Costs" (the copy-based versus zero-copy taxonomy), and "NATS JetStream: the lightweight Kafka alternative, and why durability disqualifies it for security data" (the full Jepsen analysis).
