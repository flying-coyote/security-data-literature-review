...
- [ ] **Add a LICENSE file.** The repo has none as of 2026-07-12, and Zenodo requires a license selection at deposit while `.zenodo.json` deliberately omits the `license` key rather than invent one. CC-BY-4.0 is the usual choice for a text review and matches OUP Gold-OA norms, but the call is yours. After adding the file, set the matching key in both metadata files: `"license": "cc-by-4.0"` (or the chosen SPDX id, lowercased per Zenodo convention) in `.zenodo.json`, and `license: CC-BY-4.0` in `CITATION.cff`.
