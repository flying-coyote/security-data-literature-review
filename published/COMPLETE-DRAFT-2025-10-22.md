...
> **Post-publication correction (2026-07-11).** Two performance figures in this 2025-10-22 draft were later found unsupported and are withdrawn: the ClickHouse "96% of queries <1s" figure (attributed to Cloudflare, but not present in the cited article, which supports 6M req/sec only) and the "Shell 57TB/day security telemetry" leg (dead-URL citation, removed in the 2026-06-05 audit). H3-PERFORMANCE-01 recomputed from 21/25 to 17/25 (High) as a result. Do not cite either figure; the corrected record is in `analysis-bundles/hypothesis-confidence-matrix.md` and `FIGURES-AND-TABLES.md`. The Cloudflare 6M req/sec and first-party lab (~7.0× storage) legs stand.
>
> **Addendum (2026-07-16).** The two withdrawals above are not the full correction record for this snapshot: the 2026-06/07 audits went on to overturn most of the other quantitative multipliers asserted in the body below, which is preserved verbatim as the historical record of what was published on 2025-10-22. Overturned legs that run un-struck in the abstract, Sections 3.3-3.4, the hypothesis summaries, and the conclusion: the IDC 2.5-3× operational-cost premium; the DORA 2.7× staffing multiplier and the "Level 4 / top 5%" skill taxonomy (the DORA 2024 report contains no such level taxonomy); the Ververica 3.2-FTE Flink staffing figure; the "Enterprise Data Quarterly" 1.5-2× infrastructure-cost figure (no such publication exists); the Gartner/phData 5.5-month implementation timeline (H-IMPL-03 now has zero surviving quantitative legs); the Confluent 45-55% operations-share-of-TCO figure; the 39%/32%/29% TCO split credited to Cloudera/Forrester TEI (the figures appear in neither TEI document); and the AWS 55% / Netflix 70-80% tiered-storage savings (H-COST-09 is re-anchored on a first-party S3 tier-price derivation). Check every quantitative multiplier in this snapshot against CHANGELOG.md before citing it, because the 2026-06/07 audits withdrew the citations behind most of them; the correction record is METHODOLOGY.md §5.3's banner and CHANGELOG.md, and current hypothesis scores (H-IMPL-01 1/5, H-IMPL-02 2/5, H-COST-09 4/5) live in LITERATURE-HYPOTHESIS-GAP-ANALYSIS.md.
...
- [57] Anonymized practitioner: Starburst/Athena viability for security operations (Level A)
